...
module.exports.run = (client, message, args) => {
	...
	(guildMembers).forEach(m_element => {
		...
		if(m_element.roles.cache.has(IDADMIN) || m_element.roles.cache.has(IDPROF) || m_element.roles.cache.has(IDBOT)) {
			console.log('Admin, aucune suppr');
		}
		else {
			(m_element.roles.cache).forEach(r_element => {
				console.log(`Suppression du role ${r_element.name} de ${m_element.user.username}`);
				if(r_element.id === IDADMIN || r_element.name === '@everyone') {
					console.log('Role admin ou everyone dodge');
				}
				else {
					m_element.roles.remove(r_element)
						.then(message.channel.send(`Suppression des rôles de ${m_element} effectuée`));
				}
			});
		}
	});
